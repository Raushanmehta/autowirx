...
// Get the existing blog items
const existingItems = data.AutoWirx.sections.blog.variants.AutoWirxBlog1.items;

// Update existing items to have slugs and content
data.AutoWirx.sections.blog.variants.AutoWirxBlog1.items = existingItems.map(item => {
  const slug = createSlug(item.title);
  
  // If this is the specific post we have detailed content for, add it, otherwise add dummy content
  if (item.title === "5 Signs It's Time to Replace Your Phone Screen") {
    return {
      ...item,
      href: `/blog/${slug}`,
    ...
    // Add generic dummy content for other blogs so they don't break
    return {
      ...item,
      href: `/blog/${slug}`,
      author: 'Auto Wirx Team',
      ...
      contentBlocks: [
        {
          title: 'Introduction',
          content: 'This is a placeholder for the detailed article content. We will update this soon with comprehensive guides and tips.'
        }
      ]
    };
  }
...
// Also move sidebar data to blog section
data.AutoWirx.sections.blog.variants.AutoWirxBlog1.sidebar = {
  categories: [
...
// Delete blogDetails
delete data.AutoWirx.sections.blogDetails;

fs.writeFileSync(file, JSON.stringify(data, null, 2));
console.log('Moved blog details into blog items and cleaned up');
